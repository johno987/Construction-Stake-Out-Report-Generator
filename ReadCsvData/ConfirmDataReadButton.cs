...
public partial class StakeOutReport
{
    private void ConfirmChoiceAndReadButton_Click(object sender, EventArgs e) //once confirm button has been pressed
    {
        ...
        if (!string.IsNullOrEmpty(DesignDataFilePath) && !string.IsNullOrEmpty(AsBuiltDataFilePath))
        {
            ...
            catch (Exception) //catches any errors in reading the data (empty, incorrect info etc) will catch the bad read and advise the user the input file is bad
            {
    ...
    private List<Point> RemoveControlPoints()
    {
        ...
        var sanitisedPoints = AsBuiltData
            .Where(asBuiltPoint =>
                designIDList.Contains(RemovePrefix(asBuiltPoint.PointID, AsBuiltPrefixSelection))
            )
            .ToList();
