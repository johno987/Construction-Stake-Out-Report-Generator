        ...
        try
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Cell("D2").Value =
                    DefinedErrorTolerance == null ? "N/A" : DefinedErrorTolerance;
                worksheet
                    .Cell("D2")
                    .Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);
                ...
                for (int i = 0; i < subHeaders3D.Length; i++)
                {
                    ...
                    worksheet.Cell(5, i + 1).Value = Error2DCheckBox.Checked
                        ? subHeaders2D[i]
                        : subHeaders3D[i];
                }
                ...
                //NOW GET THE RANGE OF CELLS THAT CONTAIN THE ERROR COLUMN AND SET THE ERROR COLOUR
                if (DefineErrorCheckBox.Checked && !string.IsNullOrEmpty(ToleranceTextBox.Text))
                {
                    ...
                    foreach (var cell in ErrorRangeData.Cells())
                    {
                        ...
                        {
                            worksheet
                                .Range($"A{cell.Address.RowNumber}:{cell.Address}")
                                .Style.Fill.BackgroundColor = XLColor.Jasper;
                        }
                    }
                }
